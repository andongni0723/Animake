...
func main():
    ...
    a = Flow.all()
    a.add(panel.tween.size.x.to(1500).span(0.8))
    a.add(panel.tween.self_color.to(BLUE).span(0.8))
    a.add(panel.tween.rotation_degrees.to(-5.0).span(0.8))
    a.add(text.tween.label.text.to("ANIMATION").span(0.5))
    a.add(text.tween.size.x.to(1500).span(0.8))
    a.wait().end()

    a = Flow.all()
    a.add(panel.tween.size.x.to(1000).span(0.8))
    a.add(panel.tween.self_color.to(YELLOW).span(0.8))
    a.add(panel.tween.rotation_degrees.to(0.0).span(0.8))
    a.add(text.tween.label.text.to("END").span(0.5))
    a.add(text.tween.size.x.to(1000).span(0.8))
    a.wait().end()
