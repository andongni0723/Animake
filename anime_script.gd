...
func _player_move_and_jump():
	...
	Core.tween(player, "pos_x",	0, 1, Tween.TRANS_LINEAR)

	await Core.tween_and_wait(player, "pos_y", -250, 0.5, Tween.TRANS_QUART, Tween.EASE_OUT)
	await Core.tween_and_wait(player, "pos_y", -140, 0.5, Tween.TRANS_QUART, Tween.EASE_IN)
